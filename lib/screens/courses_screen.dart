...
/// واجهة قائمة الدورات الشاملة
/// تم تثبيت الصور في جهة اليمين وتحديث أيقونة الرجوع لتطابق النظام الجديد
class CoursesScreen extends StatelessWidget {
  const CoursesScreen({super.key});

  @override
  Widget build(BuildContext context) {
    // بيانات الدورات مع الصور الحقيقية
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text("الدورات المتاحة", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 22)),
          centerTitle: false,
        ),
        body: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return GestureDetector(
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (c) => CourseDetailsScreen(courseData: course))),
  ...
  Widget _buildCourseCard(Map<String, dynamic> course) {
    return Container(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            // 1. الصورة جهة اليمين (في RTL) مع إطار أصفر
            _buildCourseImage(course['img']),
            const SizedBox(width: 15),
            // 2. المعلومات في المنتصف
  ...
  Widget _buildCourseImage(String? imagePath) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0xFFFBEC21).withValues(alpha: 0.5), width: 1.5),
  ...
  Widget _buildCourseInfo(Map<String, dynamic> course) {
    return Column(
      ...
      children: [
        ...
        Text(course['cert'], style: const TextStyle(fontSize: 11, color: Colors.grey)),
  ...
  Widget _buildPrice(String price) {
    return Text(price, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.black87));
  }
}
