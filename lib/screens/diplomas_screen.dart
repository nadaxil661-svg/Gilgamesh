...
/// واجهة قائمة الدبلومات المهنية
/// تم تحديث أيقونة الرجوع لتطابق الشكل المطلوب (arrow_back في RTL تشير لليمين)
class DiplomasScreen extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
          IconButton(
            ...
            // استخدام arrow_back التي تشير لليمين في وضع RTL
  ...
  /// بناء قائمة الدبلومات الـ 8 الكاملة
  ...
  Widget _buildDiplomaCard(BuildContext context, Map<String, dynamic> item) {
    return GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => DiplomaDetailsScreen(diplomaData: item))),
      child: Container(
        height: 100,
  ...
  Widget _buildDiplomaIcon(String imagePath) {
    return SizedBox(
      width: 50,
      height: 50,
      child: Image.asset(imagePath, fit: BoxFit.contain, errorBuilder: (c, e, s) => const Icon(Icons.school, color: Colors.grey)),
    );
  }

  Widget _buildDividerLine(Color color) {
    return Container(width: 1.5, height: 40, margin: const EdgeInsets.symmetric(horizontal: 15), color: color.withValues(alpha: 0.3));
  }
  ...
  Widget _buildDiplomaInfo(String title, String duration, String subjects) {
    return Column(
      ...
      children: [
        ...
        Text("$duration | $subjects", style: const TextStyle(fontSize: 11, color: Colors.black45)),
      ],
    );
  }

  Widget _buildPrice(String price) {
    return Text(price, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87));
  }
}
